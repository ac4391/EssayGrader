...
class RNN():
    def __init__(self, num_classes=12, embed_size=100, batch_size=64, num_steps=500, cell_type='LSTM',
                 rnn_size=128, num_layers=2, learning_rate=0.001, training=True):
        '''
        Initialize the input parameter to define the network
        inputs:
        :param embed_size: (int) embeding size of the word vector
    ...
    def inputs_layer(self):
        ...
        self.inputs = tf.placeholder(tf.float32, shape=(self.batch_size, self.num_steps, self.embed_size), name='inputs')
        self.targets = tf.placeholder(tf.float32, shape=(self.batch_size, 1), name='targets')
